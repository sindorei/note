...
## 难题
- 自然界的失误都不是以无限平铺的方式存在的
- CSS本身没有提供任何随机功能


## 解决方案
- 从一个平面拆散为多个图层：一个颜色作为底色，另三种颜色作为条纹，然后再让条纹以不同的间隔进行重复平铺
- 应该把平铺间距最大的贴片安排在最顶层
- 贴片的尺寸实际上就是所有`background-size`的最小公倍数
- 要让随机性更加真实得把贴片的尺寸最大化
- 为了让最小公倍数最大化，这些数字做好是相对质数
- 蝉原则，通过质数来增加随机真实性
  * Alex Walker的《蝉原则对网页设计的重要性》


# 连续的图像边框
> 背景知识：CSS渐变，基本的border-image，条纹背景，基本的css动画
